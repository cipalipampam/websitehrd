...
export const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Check if user role is HR after successful login
      const currentUser = useAuthStore.getState().user;

      if (currentUser && currentUser.role !== "HR") {
        // Logout the user if role is not HR
        useAuthStore.getState().logout();
        setError("Access denied. Only HR personnel are allowed to login.");
        return;
      }

      // If role is HR, proceed to dashboard
      navigate("/dashboard");
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex relative">
      ...
      {isDarkMode && (
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at center, transparent 10%, rgba(0,0,0,0.1) 30%, rgba(0,0,0,0.2) 100%)",
          }}
        />
      )}
        ...
        <div className="w-full max-w-md relative z-40">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {error && (
              <Alert
                variant="destructive"
                className="border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/50"
              >
      ...
      <div className="hidden lg:flex flex-1 bg-transparent items-center justify-center relative overflow-hidden z-10">
        ...
        {!isDarkMode && (
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, rgba(255,255,255,0.9) 0%, rgba(255,255,255,0.3) 30%, rgba(255,255,255,0.1) 70%, rgba(255,255,255,0.8) 100%)",
            }}
          />
        )}
        ...
        {isDarkMode && (
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, rgba(15,23,42,0.9) 0%, rgba(15,23,42,0.3) 30%, rgba(15,23,42,0.1) 70%, rgba(15,23,42,0.8) 100%)",
            }}
          />
        )}
        ...
        <div className="relative z-40 text-center">
          ...
          <p className="text-gray-800 dark:text-gray-200 max-w-sm font-bold">
            Streamline your human resources with our comprehensive management
            system
          </p>
        </div>
      </div>
